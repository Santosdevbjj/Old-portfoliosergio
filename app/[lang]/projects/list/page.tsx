...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `${baseUrl}/${lang}/projects/list`,
      languages: {
        pt: `${baseUrl}/pt/projects/list`,
        en: `${baseUrl}/en/projects/list`,
        es: `${baseUrl}/es/projects/list`,
      },
    },
    openGraph: {
      title: `${dict.sections.featuredProjects} | Sérgio Santos`,
...
export default async function ProjectsListPage({ params }: PageProps) {
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-slate-950 pt-20">
      ...
      {/* Grid */}
      <section className="container mx-auto max-w-6xl px-6 lg:px-8 py-20">
        <div className="flex items-center justify-between mb-12 border-l-4 border-blue-600 pl-6">
          ...
          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest bg-slate-100 dark:bg-slate-900 px-3 py-1 rounded-lg">
            {projects.length} {dict.common.projects}
          </span>
        </div>

        {projects.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-slate-200 dark:border-slate-800/50 rounded-[3rem] bg-slate-50/50 dark:bg-slate-900/5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.map((repo, index) => (
              ...
              >
                <ProjectCard repo={repo} dict={dict} lang={lang} />
              </div>
            ))}
          </div>
        )}
